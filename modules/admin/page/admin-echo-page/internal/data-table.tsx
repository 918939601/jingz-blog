            ...
            {table.getHeaderGroups().map(headerGroup => (
              <TableRow key={headerGroup.id}>
                {headerGroup.headers.map((header) => {
                  return (
                    <TableHead key={header.id} className="text-foreground/48 dark:text-white/64">
                  ...
                  table.getRowModel().rows.map(row => (
                    ...
                    >
                      {row.getVisibleCells().map(cell => (
                        <TableCell key={cell.id} className="max-w-24 truncate text-foreground/74">
                          {flexRender(
                  ...
                  <TableRow>
                    ...
                    >
                      <div className="flex min-h-32 flex-col items-center justify-center rounded-[18px] border border-dashed border-black/8 bg-black/[0.015] px-4 text-center dark:border-white/10 dark:bg-white/[0.015]">
                        ...
                        <p className="paper-title mt-4 text-2xl">暂时没有引用</p>
                        <p className="mt-2 text-sm text-foreground/52">把最近想留下的一句话收进来，后台会帮你整理展示。</p>
                      </div>
                    </TableCell>
                  </TableRow>
                )}
